...
def getter_for_related_field(name, admin_order_field=None,
    ...
    getter.short_description = (short_description
                                or related_names[-1].title().replace('_', ' '))
    return getter


class RelatedFieldAdminMetaclass(type(admin.ModelAdmin)):
    def __new__(cls, name, bases, attrs):
        new_class = super(RelatedFieldAdminMetaclass, cls).__new__(cls, name,
                                                                   bases,
                                                                   attrs)
...
class RelatedFieldAdmin(with_metaclass(RelatedFieldAdminMetaclass,
                                       admin.ModelAdmin)):
    def get_queryset(self, request):
        qs = super(RelatedFieldAdmin, self).get_queryset(request)
        select_related = ([field.rsplit('__', 1)[0]
